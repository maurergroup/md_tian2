...
contains
    ...
    subroutine read_poscar(teil, slab, infile)
        ...
        use run_config, only : simparams
        use useful_things, only : file_exists, split_string
        use open_file, only : open_for_read
        ...
        read(geo_unit, *) scaling_const             ! second line is scaling constant

!        print *, buffer
!        print *, scaling_const

        ! read the cell vectors
        read(geo_unit, *) simbox(:,:)

        ! read atom types
        read(geo_unit, '(A)') buffer !(elements(i), i=1,nspecies)
        call split_string(buffer, elements(:), nwords)

        ! read number of atoms and number of beads
        read(geo_unit, '(A)') buffer
        ...
        if (pos1 > 0) then  ! Number of beads is stated in the file
            ...
            if (ios /= 0 .or. nbeads < 1) stop "Error reading number of beads from poscar file"
        end if
        read(buffer(pos2+1:), *) natoms(:)

        ! now we have everything to construct atom types
        if (simparams%nprojectiles > 0) teil = new_atoms(nbeads, sum(natoms(:simparams%nprojectiles)))
        if (simparams%nlattices    > 0) slab = new_atoms(nbeads, sum(natoms(simparams%nprojectiles+1:)))

        ! Direct or Cartesian coordinates, needed later
        read(geo_unit, '(A)') c_system
        ...
        read(geo_unit, '(A)', advance='no', iostat=ios) buffer
        backspace(geo_unit)
        call split_string(buffer, words, nwords)

        if (nwords == 3) then

            if (simparams%nprojectiles > 0) read(geo_unit, *) teil%r
            if (simparams%nlattices    > 0) read(geo_unit, *) slab%r

        else if (nwords == 6) then

            if (simparams%nprojectiles > 0) then
                allocate (can_move(3, nbeads, sum(natoms(:simparams%nprojectiles))))
                read(geo_unit, *) ((teil%r(:,j,i), can_move(:,j,i), i=1,sum(natoms(:simparams%nprojectiles))), j=1,nbeads)
                where (can_move == "F") teil%is_fixed = .true.
                deallocate (can_move)
            end if

            if (simparams%nlattices > 0) then
                allocate(can_move(3, nbeads, sum(natoms(simparams%nprojectiles+1:))))
                read(geo_unit, *) ((slab%r(:,j,i), can_move(:,j,i), i=1,sum(natoms(:simparams%nlattices))), j=1,nbeads)
                where (can_move == "F") slab%is_fixed = .true.
            end if

        else
            stop "Error: cannot read coordinate section in poscar file"
        end if

        !TODO multiply lattice constant
        !TODO invert cell matrix
        !TODO read velocity secion
