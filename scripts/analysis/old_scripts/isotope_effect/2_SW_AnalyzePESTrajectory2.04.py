...
def initialize():
	...
	for linenum,line in enumerate(mxt_file):
		...
		if this_traj.ekin_p_f < 1.1*this_traj.ekin_p_i:
			traj_list.append(this_traj)
                        if this_traj.has_transmitted is True:
                            print "Particle was transmitted in traj number {}".format(linenum)
                        counter += 1

                else:
                        counter_to_total += 1
			print "Neglecting traj number {} w/ final ekin of {}".format(linenum, this_traj.ekin_p_f)

                if this_traj.r_p_f.z > 3.6 and this_traj.cl_appr < 1.4:
                        #print("Analyze slow component in traj {}".format(linenum))
                        cl_appr_count += 1
        ...
        print("Scattering after barrier {}".format(float(cl_appr_count)/float(ntrajs)))
        #print("Final Pos number {}".format(r_p_min_count))
        print("Traj with adsorption {}".format(float(adsorb_count)/float(ntrajs)))
	mxt_file.close()
