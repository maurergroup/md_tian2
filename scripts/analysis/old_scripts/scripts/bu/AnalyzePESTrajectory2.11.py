...
#METAL_TYPE = "Xe"
#SHOT_THRU_LIMIT = -17.56781462252
...
def initialize():
	...
	for line in mxt_file:
		...
		time_int = float(sl[24])					#	time_int = 48.2000000
...
def analyze(trajs):
	...
	if len(energy_collect) != 0:
		...
		for i in range(len(occurence_hist)):
			...
			occurence_file.write("%f %d\n" % (this_angle, occurence_hist[i]/(2.0*SPECULAR_RADIUS)))
	else:
		ang_dist_file.write("%f %f %d\n"   % (0.1, 0.5, 0))
                ang_dist_file.write("%f %f %d\n\n" % (0.1, 1.0, 1))
		ang_dist_file.write("%f %f %d\n"   % (0.2, 1.0, 1))
	...
	spherical_file = open("analysis/rel_spherical_symmetry_matrix.txt", "w")
	spherical_file.write("# x-range in degrees (left to right) from %f to %f in steps of %f\n" % (0.5*(rel_xedges[0]+rel_xedges[1]), 0.5*(rel_xedges[-2]+rel_xedges[-1]), abs(rel_xedges[0]-rel_xedges[1])))
	spherical_file.write("# y-range in degrees (top to bottom) from %f to %f in steps of %f\n" % (0.5*(    yedges[0]+    yedges[1]), 0.5*(    yedges[-2]+    yedges[-1]), abs(    yedges[0]-    yedges[1])))
	spherical_file.write("specular direction is %f degrees in x, and %f degrees in y\n" % (0.0, trajs[0].polar_i))
	for i in range(len(rel_spherical_hist[0])):
		for j in range(len(rel_spherical_hist)):
			spherical_file.write("%d " % rel_spherical_hist[j][i])
		spherical_file.write("\n")
	spherical_file.close()
	...
	### Eloss vs interaction time ###
	print "Calculating interaction time dependent energy loss"
	# LOOP
	time_int_all = [traj.time_int for traj in trajs if traj.has_scattered]
	time_int_ip  = [traj.time_int for traj in trajs if traj.has_scattered and traj.in_plane]

	# ANALYSIS
	time_int_all_hist, time_int_all_xedges, time_int_all_yedges = numpy.histogram2d(time_int_all, all_eloss,          bins=numbins(time_int_all), normed=False)
	time_int_ip_hist,  time_int_ip_xedges,  time_int_ip_yedges  = numpy.histogram2d(time_int_ip,  in_plane_all_eloss, bins=numbins(time_int_all), normed=False)

	# OUTPUT
	eloss_tint_file = open("analysis/eloss_tint_all.txt", "w")
	for i in range(len(time_int_all_hist)):
		for j in range(len(time_int_all_hist[i])):
			eloss_tint_file.write("%f %f %d\n" % (0.5*(time_int_all_xedges[i]+time_int_all_xedges[i+1]), 0.5*(time_int_all_yedges[j]+time_int_all_yedges[j+1]), time_int_all_hist[i][j]))
		eloss_tint_file.write("\n")
	eloss_tint_file.close()

	eloss_tint_file = open("analysis/eloss_tint_ip.txt", "w")
	for i in range(len(time_int_ip_hist)):
                for j in range(len(time_int_ip_hist[i])):
                        eloss_tint_file.write("%f %f %d\n" % (0.5*(time_int_ip_xedges[i]+time_int_ip_xedges[i+1]), 0.5*(time_int_ip_yedges[j]+time_int_ip_yedges[j+1]), time_int_ip_hist[i][j]))
                eloss_tint_file.write("\n")
        eloss_tint_file.close()
