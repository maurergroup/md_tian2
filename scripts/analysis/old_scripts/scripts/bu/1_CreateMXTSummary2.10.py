	...
	for folder in folder_list:
		...
		for line in infile:
			try:
				...
				r_p_min  = line.strip(' \n\t\r').split()[-3:]; line=infile.next()	#	r_min_p  =     51.3246421    33.1600141     0.9996723
				time_int = line.strip(' \n\t\r').split()[-1]				#	time_int =     34.8000000
...
def write_summary():
	...
	outfile.write("# E_kin_p   E_kin_l        E_pot      E_total r_p(    x,        y,         z)     polar       azi   E_kin_p  E_kin_l       E_pot       E_total     r_p(x,       y,         z)       polar       azi     simtime turn_pnts   cl_appr   r_p_min   time_int\n")
	for traj in traj_list:
		try:
			outfile.write("%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n" \
				% ( traj.ekin_p_i, traj.ekin_l_i, traj.epot_i, traj.etotal_i, traj.r_p_i[0], traj.r_p_i[1], traj.r_p_i[2], \
				...
				traj.r_p_min[0], traj.r_p_min[1], traj.r_p_min[2], traj.time_int))
		except (IndexError):
			outfile.close()
